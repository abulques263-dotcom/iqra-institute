...
import { Sparkles, CheckCircle2, ArrowRight, ShieldCheck, HelpCircle, Layers } from 'lucide-react';
...
export const ClassesFeesSection: React.FC<ClassesFeesProps> = ({
  classes,
  onSelectClassForTrial
}) => {
  // Sort by order
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black font-['Outfit'] text-slate-900 tracking-tight">
            Classes & Monthly Fees
          </h2>
          <p className="text-base sm:text-lg text-slate-600 font-medium">
            Clear, honest pricing with zero hidden charges. Monthly payment format for family convenience.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {sortedClasses.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`relative flex flex-col justify-between rounded-2xl bg-white p-6 sm:p-7 transition-all duration-300 ${
                  isFeatured
                    ? 'border-2 border-amber-500 shadow-lg ring-1 ring-amber-400/30'
                    : 'border border-stone-200 shadow-sm hover:shadow-md'
                }`}
              >
                {/* Popular Badge */}
                {isFeatured && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-amber-600 text-white text-[11px] font-bold uppercase tracking-wider px-3.5 py-0.5 rounded-full shadow-xs flex items-center gap-1">
                    <Sparkles className="w-3 h-3 text-amber-200" /> Most Popular
                  </div>
                )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs font-semibold text-amber-800 tracking-wide mt-0.5">
                      {item.gradeRange || 'Foundation Batch'}
                    </p>
                  </div>
                      ...
                      {item.features && item.features.length > 0 ? (
                        ...
                        <>
                          <li className="flex items-center gap-2">
                            ...
                            <span>Daily 1.5 Hours Structured Class</span>
                          </li>
                          <li className="flex items-center gap-2">
                            <CheckCircle2 className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                            <span>Core Concept Worksheets</span>
                          </li>
                          <li className="flex items-center gap-2">
                            <CheckCircle2 className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                            <span>Weekly Assessment & Revision</span>
                          </li>
                        </>
                      )}
                    </ul>
                ...
                <div className="pt-6 mt-6 border-t border-stone-100">
                  <button
                    type="button"
                    onClick={() => onSelectClassForTrial(item.name)}
                    className={`w-full py-2.5 px-4 rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-1.5 ${
                      isFeatured
                        ...
                        : 'bg-stone-900 hover:bg-slate-800 text-white'
                    }`}
                  >
        ...
        <div className="mt-12 text-center bg-white p-5 rounded-2xl border border-stone-200/80 shadow-xs max-w-3xl mx-auto space-y-2">
          <p className="text-sm font-bold text-slate-800 font-['Plus_Jakarta_Sans']">
            “Affordable monthly learning support designed for families.”
          </p>
          <p className="text-xs text-slate-500 leading-relaxed">
            All fees are charged purely on a monthly basis without long-term contracts. Parents are welcome to visit or message anytime to discuss their child's requirements.
          </p>
        </div>
